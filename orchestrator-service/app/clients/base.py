...
async def post_file_with_retry(
    client: httpx.AsyncClient,
    url: str,
    files: dict[str, Any],
    *,
    ...
    """
    Same retry/error-handling behavior as post_with_retry, but for a
    multipart/form-data upload (e.g. forwarding an uploaded PDF) instead
    of a JSON body. Used by the document-upload flow to call
    doc-processor-api's POST /process.
    """
    last_error: Optional[str] = None
    ...
    for attempt in range(1, settings.MAX_RETRIES + 2):
        try:
            ...
            response = await client.post(url, files=files, timeout=settings.PROCESSOR_TIMEOUT_SECONDS)
            ...
            last_error = f"request timed out after {settings.PROCESSOR_TIMEOUT_SECONDS}s"
            logger.warning("[ORCHESTRATOR] %s timeout on attempt %d", service_name, attempt)
        except httpx.ConnectError as exc:
            last_error = f"connection failed: {exc}"
            logger.warning("[ORCHESTRATOR] %s unreachable on attempt %d: %s", service_name, attempt, exc)
