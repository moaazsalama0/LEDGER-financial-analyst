...
- **Document listing is best-effort.** `list_documents()` calls the
  orchestrator's `GET /documents` endpoint and falls back to local session
  data when the retrieval service is unavailable.
...
- **Tests currently cover formatting helpers only.** Full Gradio callback
  and browser-level tests still need to be added.
...
| Dashboard shows no external documents | Retrieval is unavailable or has no indexed documents; local session data still works. |
