...
async def process(
    ...
    try:
        # Docling is synchronous and CPU-heavy. Running it directly in this
        # async handler blocks the event loop, which makes /health and /ready
        # time out for the entire parse. Keep one parse at a time (the backend
        # is a shared model instance) while leaving the API responsive.
