...
def build_section_chunks(document: dict):
    ...
    for page in document.get("pages", []):
        ...
        for block in blocks:
            ...
            # Accept both the processor contract (type/text/bbox) and the
            # retrieval ingest contract (content_type/content/bounding_box).
            # Prefer content_type because OCRBlock supplies a default `type`
            # of "text" even for adapted inline table blocks.
            block_type = block.get("content_type") or block.get("type")

            if block_type == "section_header":
                continue

            text = clean_text(block.get("content") or block.get("text", ""))
            ...
            section_title = block.get("section") or section.get("title")
            ...
            for chunk_idx, chunk_text in enumerate(text_parts):
                ...
                chunks.append({
                    ...
                    "content": chunk_text,
                    "content_type": "table" if block_type == "table" else "text",

                    "bounding_box": block.get("bounding_box") or block.get("bbox"),
                    "table_id": block.get("table_id"),

                    # Internal metadata
                    "source_block_ids": [block_id],
                    "chunk_type": "table" if block_type == "table" else "text",
                })
